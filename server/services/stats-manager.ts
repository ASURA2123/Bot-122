...
class StatsManager extends EventEmitter {
  ...
  constructor() {
    ...
    logger.info('Server StatsManager initialized with max listeners: 20');
  }
    ...
    try {
      ...
      this.resetTimer = setInterval(() => {
        try {
          ...
          logger.info(`Server daily stats reset successful: ${previousMessageCount} messages, ${previousActiveUsers} users cleared`);
          this.emit('statsReset');
        } catch (error) {
          logger.error('Error resetting server daily stats:', error);
        }
      }, 24 * 60 * 60 * 1000); // Reset every 24 hours
      
      logger.info('Server daily stats reset timer initialized');
    } catch (error) {
      logger.error('Failed to initialize server daily stats reset:', error);
    }
  }
  ...
  recordMessage(message: MessageStats) {
    try {
      ...
      if (!message || !message.id || !message.senderID) {
        logger.warn('Invalid message data provided to server recordMessage');
        return;
      }
      ...
      if (this.messageCount % 1000 === 0) {
        ...
        logger.info(`Server memory usage after ${this.messageCount} messages: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)}MB`);
      }

      this.emit('messageReceived', message);
    } catch (error) {
      logger.error('Error recording message in server:', error);
    }
  }
  ...
  recordCommand(command: string) {
    try {
      if (!command) {
        logger.warn('Empty command provided to server recordCommand');
        return;
      }
      ...
      if (this.commandUsage.size >= this.MAX_COMMAND_TYPES) {
        ...
        if (!this.commandUsage.has(command)) {
          ...
          if (leastUsedCommand) {
            ...
            logger.debug(`Removed least used server command: ${leastUsedCommand} (${leastUsedCount} uses)`);
          }
        }
      }
      ...
      logger.error('Error recording command in server:', error);
    }
  }
  ...
  getBasicStats() {
    try {
      ...
      logger.error('Error getting basic server stats:', error);
  ...
  getDetailedStats() {
    try {
      ...
      logger.error('Error getting detailed server stats:', error);
  ...
  private getTimeline() {
    try {
      ...
      logger.error('Error generating server timeline:', error);
      return [];
    }
  }
  ...
  private getTopCommands() {
    try {
      ...
      logger.error('Error getting top server commands:', error);
      return [];
    }
  }
  ...
  private getHourlyActivity() {
    try {
      ...
      logger.error('Error getting server hourly activity:', error);
      return [];
    }
  }
  ...
  cleanup() {
    ...
    logger.info('Server StatsManager resources cleaned up');
  }
}
